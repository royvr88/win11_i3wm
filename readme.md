...
At least add the settings from [the example config](#1-fancywm-configuration) to your own `settings.json`. This file is found at `%LOCALAPPDATA%\Packages\2203VeselinKaraganev.FancyWM_9x2ndwrcmyd2c\LocalCache\Roaming\FancyWM` (Store install) or `%AppData%\FancyWM`.
